<template>
  <div>
    <CogIcon class="cursor-pointer hover:opacity-90" @click.native="showLiquiditySettings" />
    <div
      class="pannel swap-config-modal"
      :style="{
        'top': top,
        'right': right
      }"
      v-if="isLiquiditySettingShown"
      v-click-outside="
        () => {
          isLiquiditySettingShown = false
        }
      "
    >
...
@Component({
  ...
  computed: {
    ...
    ...mapState(['theme']),
    ...mapState('swap', [ 'configSlippagePercentage', 'config']),
  },
})
export default class LiquiditySettings extends Vue {
  isNumber!: (element: HTMLElement) => boolean
  
  @Prop({ type: String, default: '12px' }) top!: string
  @Prop({ type: String, default: '12px' }) right!: string


  isLiquiditySettingShown = false

  showLiquiditySettings() {
    this.isLiquiditySettingShown = true
  }
