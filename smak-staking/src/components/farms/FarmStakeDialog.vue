<template>
  <Dialog v-model="isFarmStakeShown" @close="closeFarmStakeDialog">
    <div class="farm-stake-dialog">
      <div class="farm-stake-dialog__header mb-4">
        <div class="farm-stake-dialog__header-title">Stake LP Tokens</div>
        <div class="farm-stake-dialog__header-close" @click="closeFarmStakeDialog">&times;</div>
      </div>

      <p class="text-secondary-400 mb-4">Stake your LP Tokens to earn SMAK</p>

      <Card class="px-3 py-3 mb-4 d-flex flex-column" secondary>
        <div class="d-flex mb-2">
          <div class="d-flex">
            <p class="font-semibold text-15px mb-0 mr-2">Stake</p>
            <p class="text-primary-1300 text-15px font-semibold mb-0">
              {{ xtz.metadata.symbol }} - {{ farm.tokenB.metadata.symbol }} - LP
            </p>
          </div>

          <p class="text-secondary-600 font-semibold text-14px mb-0 ml-auto">
            Balance: {{ getLpBalance() | readableNumber }}
          </p>
        </div>
      ...
      <button
        class="
          ...
          hover:opacity-90
        "
        @click="closeFarmStakeDialog(); stakeFarm()"
      >
        Confirm
      </button>
    </div>
...
import { IFarm } from '@/interfaces/farm.interface'
  ...
  @Component({
    ...
    computed: {
      ...mapGetters('farms', ['isFarmStakeShown']),
      ...mapState('wallet', ['tk']),
      ...mapState('farms', ['farmAddress', 'farmList']),
    },
    methods: {
      ...mapMutations('farms', ['closeFarmStakeDialog']),
      ...mapActions({
        stake: 'farms/stake',
      }),
    }
  })
export default class FarmStakeDialog extends Vue {
  tk!: TezosToolkit
  farmAddress!: string
  isFarmStakeShown!: boolean;
  closeFarmStakeDialog!: () => void
  isNumber!: (element: HTMLElement) => boolean
  farmList!: IFarm[]
  amount = 0
  xtz = TOKEN_METADATA[TOKENS.XTZ.address][TOKENS.XTZ.tokenId]

  async stakeFarm(): Promise<void> {
    await this.$store.dispatch('farms/stake', { amount: Number(this.amount), tk: this.tk })
  }
  ...
  getLpBalance(): number {
    for (const farm of this.farmList) {
      if (farm.contractAddress === this.farmAddress) return farm.balance 
    }
    return 0
  }
  ...
  useMax(): void {
    this.amount = Math.floor(this.getLpBalance() * 10**6) / 10**6 || 0
  }
}
</script>
