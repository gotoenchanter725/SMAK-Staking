...
export default class SmakBurnedVsRewardChart extends Vue {
  @Prop({ type: String, default: 'staking-rewards-chart' }) containerId!: string
  ...
  get smakBurnedList(): number[] {
    return this.chartData.map(data => data.value[0])
  }

  get stakingRewardList(): number[] {
    return this.chartData.map(data => data.value[1])
  }

  get maxValue() {
    return Math.max(...this.smakBurnedList, ...this.stakingRewardList)
  }
