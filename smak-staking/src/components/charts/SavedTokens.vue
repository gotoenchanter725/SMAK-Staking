<template>
  <div class="saved-tokens pt-6">
    <h3 class="saved-tokens__title px-6">Saved Tokens</h3>

    <Table has-index :headers="tableHeaders" :items="tableItems" :per-page="8" table-class="px-6" :is-loading="!isTokensLoaded" default-sort="priceWeekChart" />
  </div>
</template>
...
@Component({
  ...
  computed: {
    ...mapGetters('dashboard', ['watchlistTokens']),
...
export default class SavedTokens extends Vue {
  ...
  get tableItems(): ISavedTokenItem[] {
    return this.watchlistTokens.map((token) => {
  ...
  async mounted() {
    this.isTokensLoaded = !!this.watchlistTokens.length
