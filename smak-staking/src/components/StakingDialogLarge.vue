  ...
  >
    <template v-slot:activator="{ on }">
      <div class="staking-details-btn">
        <div v-on="on" :class="classname" @click="showAndSetPackInfoPack()">{{ buttonname }}</div>
      </div>
    </template>
    <div class="staking-dialog theme-colors" :class="{ ...themeClass }" :content-class="theme">
      <div class="title">
        <div class="title-text">My staking details</div>
        <v-btn v-on="on" icon text @click="showState(false)"> ✕ </v-btn>
      </div>
...
@Component
export default class StakingDialogLarge extends Vue {
  ...
  @Prop({ type: Number, default: -1 }) packInfo!: number
